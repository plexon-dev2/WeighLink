...
static void App_DisplayData(void)
{
    static uint32_t last_message_count = 0U; /* tracks CmdParser total - never resets          */
    static uint16_t string_counter     = 0U; /* what PLC sees in 40012 - resets on load change */
    ...
    if (display_counter >= APP_DISPLAY_UPDATE_INTERVAL)
    {
        ...
        /* ------------------------------------------------------------------
         * COUNTER RESET: Check if PLC has set bit 0 of REG_COUNTER_RESET
         * (register 12 / address 40013) HIGH to signal a load change.
         *
         * last_message_count is synced to total_messages so old CmdParser
         * messages are skipped after reset — preventing the counter from
         * jumping to a large number when PLC clears the bit.
         *
         * string_counter is what PLC sees in 40012 — it resets to 0 and
         * then counts 1, 2, 3... for each new message after the load change.
         *
         * ESP32 never clears REG_COUNTER_RESET — PLC owns it fully.
         * ------------------------------------------------------------------ */
        if (Modbus_IsCounterResetRequested())
        {
            Serial.printf("[APP] *** COUNTER RESET by PLC - string_counter was %u, syncing last_msg=%lu to total=%lu ***\n",
                          (unsigned int)string_counter,
                          (unsigned long)last_message_count,
                          (unsigned long)total_messages);

            last_message_count = total_messages; /* skip all old messages */
            string_counter     = 0U;             /* PLC will see 0        */
            (void)Modbus_SetStringCounter(0U);

            // Serial.println("[APP] Counter reset done - will count 1,2,3... on next new message");
            return;
        }
        ...
        while (last_message_count < total_messages)
        {
            ...
            /* Use string_counter (resets on load change) not last_message_count */
            weight_data.string_counter = string_counter;
            ...
            /* Log status */
            Serial.printf("[APP] NEW msg #%lu (string_counter=%u) - Flag %s\n",
                          (unsigned long)last_message_count,
                          (unsigned int)string_counter,
