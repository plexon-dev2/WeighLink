 ...
 * @details Reads bit 0 of REG_COUNTER_RESET (register 12 / address 40013).
 *          ESP32 never writes to this register - PLC owns it fully.
 */
bool Modbus_IsCounterResetRequested(void)
{
    ...
    return ((holding_registers[REG_COUNTER_RESET] & 0x0001U) != 0U);
}
...
/**
 * @brief Handle Write Single Register (FC06)
 * @details Only REG_COUNTER_RESET (reg 12 / 40013) is writable.
 *          All other registers return Modbus exception 0x02.
 *          PLC writes 0x0001 to trigger counter reset.
 *          PLC writes 0x0000 to release - ESP32 resumes counting from 0.
 *          ESP32 never modifies this register.
 *          Response: echo the request frame (standard FC06).
 */
static void Modbus_HandleWriteSingleRegister(uint8_t *frame, uint16_t length)
{
    uint16_t reg_addr;
    ...
    if (length != 8)
    {
        Modbus_SendException(MODBUS_FC_WRITE_SINGLE_REG, MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE);
        return;
    }

    reg_addr  = ((uint16_t)frame[2] << 8) | (uint16_t)frame[3];
    reg_value = ((uint16_t)frame[4] << 8) | (uint16_t)frame[5];

    /* Only REG_COUNTER_RESET is writable */
    if (reg_addr != (uint16_t)REG_COUNTER_RESET)
    {
        Serial.printf("[MODBUS] FC06 rejected: register %d is read-only\n", (int)reg_addr);
        Modbus_SendException(MODBUS_FC_WRITE_SINGLE_REG, MODBUS_EXCEPTION_ILLEGAL_DATA_ADDRESS);
        statistics.invalid_requests++;
        return;
    }

    /* Store value - App layer reads bit 0 via Modbus_IsCounterResetRequested() */
    holding_registers[REG_COUNTER_RESET] = reg_value;

    Serial.printf("[MODBUS] FC06: REG_COUNTER_RESET = 0x%04X%s\n",
                  (unsigned int)reg_value,
                  ((reg_value & 0x0001U) != 0U) ? " -> COUNTER RESET" : " -> RESET RELEASED");

    /* Echo request frame back (standard FC06 response) */
    response[0] = (uint8_t)MODBUS_SLAVE_ID;
    response[1] = (uint8_t)MODBUS_FC_WRITE_SINGLE_REG;
    response[2] = frame[2];
    response[3] = frame[3];
    response[4] = frame[4];
    response[5] = frame[5];

    crc         = Modbus_CalculateCRC(response, 6U);
    response[6] = (uint8_t)(crc & 0xFFU);
    response[7] = (uint8_t)((crc >> 8U) & 0xFFU);

    Modbus_SendResponse(response, 8U);
    statistics.successful_writes++;
}

/**
 * @brief Handle Write Multiple Registers (FC16 / 0x10)
 * @details Used by tools like ModbusView which send FC16 instead of FC06.
 *          Only REG_COUNTER_RESET (reg 12 / 40013) is writable.
 *
 *          FC16 frame: [slave, 0x10, addr_hi, addr_lo, count_hi, count_lo,
 *                       byte_count, data_hi, data_lo, ..., crc_lo, crc_hi]
 *          Response:   [slave, 0x10, addr_hi, addr_lo, count_hi, count_lo,
 *                       crc_lo, crc_hi]
 */
static void Modbus_HandleWriteMultipleRegisters(uint8_t *frame, uint16_t length)
{
    ...
    start_addr = ((uint16_t)frame[2] << 8) | (uint16_t)frame[3];
    reg_count  = ((uint16_t)frame[4] << 8) | (uint16_t)frame[5];
    byte_count = frame[6];

    /* Only allow writing to REG_COUNTER_RESET */
    if (reg_count != 1U || start_addr != (uint16_t)REG_COUNTER_RESET)
    {
        Serial.printf("[MODBUS] FC16 rejected: only REG_COUNTER_RESET (reg 12) is writable\n");
    ...
    reg_value = ((uint16_t)frame[7] << 8) | (uint16_t)frame[8];
    holding_registers[REG_COUNTER_RESET] = reg_value;

    Serial.printf("[MODBUS] FC16: REG_COUNTER_RESET = 0x%04X%s\n",
                  (unsigned int)reg_value,
                  ((reg_value & 0x0001U) != 0U) ? " -> COUNTER RESET" : " -> RESET RELEASED");

    response[0] = (uint8_t)MODBUS_SLAVE_ID;
    response[1] = (uint8_t)MODBUS_FC_WRITE_MULTIPLE_REGS;
    response[2] = frame[2];
    response[3] = frame[3];
    response[4] = frame[4];
    response[5] = frame[5];
